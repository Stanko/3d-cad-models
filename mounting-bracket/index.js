...
const main = (params) => {
  ...
  const walls = [
    // bottom
    cuboid({
      size: [bottom.depth, bottom.width, bottom.height],
      center: [0, 0, params.wallThickness / 2],
    }),
    // sides
    cuboid({
      size: [side.depth, side.width, side.height],
      center: [0, bottom.width / 2 - halfThickness, side.height / 2],
    }),
    cuboid({
      size: [side.depth, side.width, side.height],
      center: [0, bottom.width / -2 + halfThickness, side.height / 2],
    ...
    // top arms
    cuboid({
      size: [arm.depth, arm.width, arm.height],
      center: [0, (bottom.width - arm.width) / 2, side.height - halfThickness],
    }),
    cuboid({
      size: [arm.depth, arm.width, arm.height],
      center: [0, (bottom.width - arm.width) / -2, side.height - halfThickness],
...
const getParameterDefinitions = () => {
  return [
    ...
    },
    {
      ...
      min: 10,
      max: 45,
      caption: 'Arms length (10 - 45%)',
    },
    {
      name: 'bracketWidth',
      type: 'float',
      initial: 30,
      caption: 'Bracket Width',
    },
    {
      name: 'holeDiameterTop',
      type: 'float',
      initial: 12,
      caption: 'Hole top diameter',
    },
    {
      name: 'holeDiameterBottom',
      type: 'float',
      initial: 6,
      caption: 'Hole bottom diameter',
    },
  ];
};
